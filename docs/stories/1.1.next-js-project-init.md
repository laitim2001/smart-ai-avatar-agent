...
- [ ] **Task 2: 開發工具鏈配置** (AC: 3)
  - [ ] 安裝 ESLint 與 Prettier：`npm install -D prettier eslint-config-prettier`
...
- [ ] **Task 4: 環境變數與文件準備** (AC: 5, 6)
  ...
  - [ ] 建立 `README.md`，包含以下內容：
    ...
    - 安裝步驟：`npm install`, 設定 `.env.local`, `npm run dev`
    - 執行指令：開發、建置、測試
    - 專案結構說明（簡要）
...
- ✅ 驗證 TypeScript 編譯無錯誤（`npm run build`）
- ✅ 驗證本地開發伺服器啟動成功（`npm run dev`）
